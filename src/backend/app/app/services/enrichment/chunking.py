...
class ChunkingService:
        ...
        for idx, translation in enumerate(translations):
            ...
            if translation_notes and isinstance(translation_notes, list):
                for note_idx, note in enumerate(translation_notes):
                    ...
                    typed_note = cast(dict[str, Any], note)
                    note_text = str(typed_note.get("note", "") or "")
                    ...
                    line_value = typed_note.get("line")
                    if line_value is not None:
                        note_metadata["lines"] = line_value
                            ...
                            if isinstance(subjects, list):
                                for subj_idx, subj in enumerate(subjects):
                                    if isinstance(subj, dict):
                                        subject = cast(dict[str, Any], subj)
                                        subj_parts = []

                                        subject_level_1 = subject.get('subjectLevel1')
                                        if subject_level_1:
                                            subj_parts.append(f"Subject: {subject_level_1}")
                                        subject_level_2 = subject.get('subjectLevel2')
                                        if subject_level_2:
                                            subj_parts.append(f"Detail: {subject_level_2}")
                                        view = subject.get('view')
                                        if view:
                                            subj_parts.append(f"View: {view}")

                                        human_gender = subject.get('humanGender')
                                        if human_gender:
                                            subj_parts.append(f"Gender: {human_gender}")
                                        part_of_human_body = subject.get('partOfHumanBody')
                                        if part_of_human_body:
                                            subj_parts.append(f"Body part: {part_of_human_body}")

                                        clothes = _string_list(subject.get('humanClothes'))
                                        if clothes:
                                            subj_parts.append(f"Clothes: {', '.join(clothes)}")

                                        weapons = _string_list(subject.get('humanWeapons'))
                                        if weapons:
                                            subj_parts.append(f"Weapons: {', '.join(weapons)}")

                                        gestures = _string_list(subject.get('humanGestures'))
                                        if gestures:
                                            subj_parts.append(f"Gestures: {', '.join(gestures)}")

                                        jewellery = _string_list(subject.get('humanJewellery'))
                                        if jewellery:
                                            subj_parts.append(f"Jewellery: {', '.join(jewellery)}")

                                        part_of_animal_body = subject.get('partOfAnimalBody')
                                        if part_of_animal_body:
                                            subj_parts.append(f"Animal part: {part_of_animal_body}")

                                        animal_gestures = _string_list(subject.get('animalGestures'))
                                        if animal_gestures:
                                            subj_parts.append(f"Animal gestures: {', '.join(animal_gestures)}")

                                        symbol_shape = subject.get('symbolShape')
                                        if symbol_shape:
                                            subj_parts.append(f"Symbol: {symbol_shape}")
                                        symbol_reference = subject.get('symbolReference')
                                        if symbol_reference:
                                            subj_parts.append(f"Reference: {symbol_reference}")
                                        symbol_reference_text = subject.get('symbolReferenceText')
                                        if symbol_reference_text:
                                            subj_parts.append(f"({symbol_reference_text})")

                                        monogram_name = subject.get('monogramName')
                                        if monogram_name:
                                            subj_parts.append(f"Monogram: {monogram_name}")
    ...
    def _semantic_chunking_with_embeddings(
        ...
        try:
            embeddings = embeddings_service.generate_embeddings_batch(sentences)
        except Exception as e:
            logging.error(f"Error generating sentence embeddings: {e}")
            return self._token_based_chunking(sentences, chunk_type, metadata)

        if any(embedding is None for embedding in embeddings):
            logging.warning("Failed to generate one or more sentence embeddings, falling back")
            return self._token_based_chunking(sentences, chunk_type, metadata)

        non_null_embeddings = [embedding for embedding in embeddings if embedding is not None]
        if len(non_null_embeddings) != len(embeddings):
            logging.warning("Failed to generate one or more sentence embeddings, falling back")
            return self._token_based_chunking(sentences, chunk_type, metadata)

        sentence_embeddings = [np.array(embedding) for embedding in non_null_embeddings]
    ...
    def create_chunks_for_epigraph(
        ...
        defer_embedding_generation: bool = False,
    ) -> List[EpigraphChunk]:
        """Create and return chunk objects for an epigraph."""
        if epigraph.id is None:
            raise ValueError("Epigraph must be persisted before chunking")
        ...
        if generate_embeddings and not defer_embedding_generation:
            embeddings_service = EmbeddingsService(self.session)
        ...
        if embeddings_service and chunk_dicts:
            try:
                embeddings = embeddings_service.generate_embeddings_batch(
                    [chunk_dict["text"] for chunk_dict in chunk_dicts]
                )
                for chunk, embedding in zip(chunk_objects, embeddings):
                    chunk.embedding = embedding
                    if embedding is None:
                        logging.warning(
                            "Failed to generate embedding for chunk %s on epigraph %s",
                            chunk.chunk_index,
                            epigraph_id,
                        )
            except Exception as e:
                logging.error(f"Failed to generate chunk embeddings in batch: {e}")
    ...
    def create_and_save_chunks(
        ...
        epigraph_id = epigraph.id
        chunks = self.create_chunks_for_epigraph(
            epigraph,
            generate_embeddings,
        ...
        for chunk in chunks:
            try:
                ...
                logging.error(f"Failed to save chunk {chunk.chunk_index} for epigraph {epigraph_id}: {e}")
                raise

        if generate_embeddings and defer_embedding_generation:
            EmbeddingsService(self.session).flush_pending_chunk_embeddings(force=False)

        logging.info(f"Saved {len(saved_chunks)} chunks for epigraph {epigraph_id}")
        return saved_chunks
    ...
    def update_chunks_for_epigraph(
        ...
        epigraph_id = epigraph.id
        deleted_count = crud_epigraph_chunk.delete_by_epigraph_id(self.session, epigraph_id=epigraph_id)
        logging.info(f"Deleted {deleted_count} existing chunks for epigraph {epigraph_id}")

        return self.create_and_save_chunks(
            epigraph,
            generate_embeddings,
            defer_embedding_generation=defer_embedding_generation,
        )
    ...
    def get_chunk_statistics(self) -> Dict[str, Any]:
        ...
        total_chunks = self.session.exec(
            select(func.count(chunk_id_column))
        ).one()

        chunks_by_type = self.session.exec(
            select(
                chunk_type_column,
                func.count(chunk_id_column)
            ).group_by(chunk_type_column)
        ).all()

        avg_tokens = self.session.exec(
            select(func.avg(token_count_column))
        ).one()
